	...
	public sealed class Resolver
	{
		...
		private static byte[] UdpTransfer(IPEndPoint server, byte[] requestMessage)
		{
			...
			while (attempts <= _udpRetryAttempts)
			{
                ...
                byte[] returnResponseMessage = new byte[2048];
				...
				try
				{
                    ...
                    while((amount = socket.Receive(responseMessage)) >= 512)
                    {
                        // wait for a response upto 1 second
                        //int amount = socket.Receive(responseMessage);
                        try
                        {
                            Array.Copy(responseMessage, 0, returnResponseMessage, pos, amount);
                        }
                        catch
                        {
                            break;
                            //I don't think you receive more than 2048 bytes, and if it, fuck off xD
                        }
                        pos = amount;
                    }
                    Array.Copy(responseMessage, 0, returnResponseMessage, pos, amount);
					// make sure the message returned is ours
                    if (returnResponseMessage[0] == requestMessage[0] && returnResponseMessage[1] == requestMessage[1])
					{
						// its a valid response - return it, this is our successful exit point
                        return returnResponseMessage;
					}
				}
